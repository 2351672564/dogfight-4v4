...
def change_missile_name(missiles):
    missiles_changed=''
    missile_name_split=re.split('([0-9]+)', missiles)
    missiles_changed=missile_name_split[0]+missile_name_split[1]+'-'+missile_name_split[2]+'-'+missile_name_split[3]
...
def server_update():
	...
	threading.Thread(target=server_update, daemon=True)
...
def get_plane_state(args):
	...
	for name in planes:
		machine = main.destroyables_items[str(name)]
		...
		targets_distance = []
		targets_heading_diff = []
		if td is not None:
			target_id = td.get_target_name()
			target_locked = td.target_locked
			target_distance = hg.Len(td.targets[td.target_id - 1].parent_node.GetTransform().GetPos() - position)
			target_heading_diff = td.target_heading - machine.get_heading()
			while target_heading_diff <= -180:
				target_heading_diff += 360
			while target_heading_diff > 180:
				target_heading_diff -= 360
			for target in td.targets:
				targets_distance.append((target.id, hg.Len(target.parent_node.GetTransform().GetPos() - position)))
				diff = target.get_heading() - machine.get_heading()
				while diff <= -180:
					diff += 360
				while diff > 180:
					diff -= 360
				targets_heading_diff.append((target.id, diff))
		else:
			target_id = "- ! No TargettingDevice ! -"
			target_locked = False
			target_distance = -1
			target_heading_diff = 0

		allies_distance = []
		allies_heading_diff = []
		for ally in machine.allies:
			allies_distance.append((ally.id, hg.Len(ally.parent_node.GetTransform().GetPos() - position)))
			diff = ally.get_heading() - machine.get_heading()
			while diff <= -180:
				diff += 360
			while diff > 180:
				diff -= 360
			allies_heading_diff.append((ally.id, diff))
		...
		state = {
			...
			"allies_distance": allies_distance,
			"target_heading_diff": target_heading_diff,
			"targets_heading_diff": targets_heading_diff,
			"allies_heading_diff": allies_heading_diff,
			"actions": machine.action_map,
		}
		if flag_print_log:
			#print(args["plane_id"])
			print(str(state))
		planes_state.append(state)
	socket_lib.send_message(str.encode(json.dumps(planes_state)))
...
def get_missile_state(args):
	global planes_g,missiles_g,flag_g
	all_state=[]
	if(flag_g==False):
		for dm in main.destroyables_list:
			if dm.type == Destroyable_Machine.TYPE_AIRCRAFT:
				planes_g.append(dm.name)
			if dm.type == Destroyable_Machine.TYPE_MISSILE:
				na=change_missile_name(str(dm.name))
				if 'CFT' not in str(dm.name):
					missiles_g.append(na)
		flag_g=True
		print(planes_g)
		print(missiles_g)
...
#planes	
	for name in planes_g:
		...
		iactrl = machine.get_device("IAControlDevice")
		td = machine.get_device("TargettingDevice")
		gn=machine.get_device("MachineGunDevice_00")#需要注意飞机的机枪个数，命名规则为0x
		if gear is not None:
			gear_activated = gear.activated
		else:
			gear_activated = False
		if apctrl is not None:
			autopilot_activated = apctrl.is_activated()
			autopilot_heading = apctrl.autopilot_heading
			autopilot_pitch=apctrl.autopilot_pitch_attitude
			autopilot_roll=apctrl.autopilot_roll_attitude
			autopilot_speed = apctrl.autopilot_speed
			autopilot_altitude = apctrl.autopilot_altitude
		else:
			autopilot_activated = False
		...
		if iactrl is not None:
			ia_activated = iactrl.is_activated()
			ia_missile=iactrl.IA_fire_missile_cptr#发射完导弹后cptr会变为10，然后根据时间刻减少，作为导弹冷却
			ia_gotolanding=iactrl.IA_flag_goto_landing_approach_point
		else:
			ia_activated = False
		if td is not None:
			target_id = td.get_target_name()
		...
		#标志实现
		#heading
		diff = autopilot_heading - heading
		if diff > 180:
			diff -= 360
		elif diff < -180:
			diff += 360
		flagm_heading=diff
		#roll
		diff =autopilot_roll-roll_attitude
		diff = max(-1, min(1, diff / 20))
		flagm_roll=diff
		#pitch
		diff = autopilot_pitch-pitch_attitude
		diff = max(-1, min(1, diff / 10))
		flagm_pitch=diff
		if ia_missile>0:
			fissile=1
		else:
			fissile=0
		#意图实现
		if td.target_locking_state==0 and td.target_out_of_range==1:
			flagy_next=1
		else :
			flagy_next=0
		if 0<td.target_locking_state<1 and td.target_out_of_range==0 and target_locked==0:
			flagy_locking=1
		else:
			flagy_locking=0
		if td.target_locking_state==1 and target_locked==1:
			flagy_attack=1
		else :
			flagy_attack=0
		state = {
			"timestamp": main.framecount,#时间刻
			"type": Destroyable_Machine.types_labels[machine.type],#智能体类型
			"position": [position.x, position.y, position.z],#位置x，y，z
			"Euler_angles": [rotation.x, rotation.y, rotation.z],
			"move_vector": [v_move.x, v_move.y, v_move.z],#在空间坐标x，y，z方向上的速度
			"destroyed": machine.flag_destroyed,#坠毁标志
			"wreck": machine.wreck,#沉没标志
			"crashed": machine.flag_crashed,#撞毁标志
			"active": machine.activated,#存活标志
			"nationality": machine.nationality,#队伍
			"horizontal_speed": h_spd,#平面速度
			"vertical_speed": v_spd,#垂直速度
			"linear_speed": linear_speed,#飞机当前线速度，总速度
			"altitude": altitude,#高度
			"pitch_attitude": pitch_attitude,#俯仰姿态
			"roll_attitude": roll_attitude,#滚转姿态
			"heading": heading,#朝向姿态
			#共有特征
			"ia_activated":ia_activated,#ai启动
			"missile_fired":ia_missile,#导弹发射和导弹冷却变量
			#autopilot是一个ai控制的趋势变量，与智能体变量差距越大就意味着动作越大
			"autopilot_activated":autopilot_activated,
			"autopilot_heading":autopilot_heading,
			"autopilot_pitch":autopilot_pitch,
			"autopilot_roll":autopilot_roll,
			"autopilot_speed":autopilot_speed,
			"autopilot_altitude":autopilot_altitude,
			"name":name,#智能体姓名
			"linear_acceleration": linear_acceleration,#总加速度
			"post_combustion": machine.post_combustion,#后喷焰加速
			"gun_bullets":gnb,#子弹数量
			"gun_activated":gna,#机枪激活
			"gear": gear_activated,#起落架
			"gear_level":gear.gear_level,#起落架状态
			"target_locked": target_locked,#锁定状态
			"target_angle":td.target_angle,#目标角度
			"target_distance":td.target_distance,#目标距离
			"target_lock_time":td.target_lock_t,#锁定时间
			"target_lock_state":td.target_locking_state,#锁定中0-1
			"target_outof_range":td.target_out_of_range,#不在锁定范围内
			"health_level": machine.health_level,#耐久值
			"thrust_level": thrust_level,#当前推力系数
			"brake_level": brake_level ,#当前阻力系数
			"flaps_level": flaps_level,#襟翼度数
			#行为标志
			"flagm_heading":flagm_heading,#偏航行为
			"flagm_roll":flagm_roll,#滚转行为
			"flagm_pitch":flagm_pitch,#俯仰行为
			"flagm_gun":gna,#机炮行为
			"flagm_missile":fissile,#发射行为
			#意图标志
			"target_id": target_id,#意图目标
			"flagy_next":flagy_next,#追击意图
			"flagy_locking":flagy_locking,#锁定意图
			"flagy_attack":flagy_attack,#攻击意图
			"flagy_gotolanding":ia_gotolanding,#休整意图
			#主要注意变量
			"active": machine.activated,#存活变量
		}
		if flag_print_log:
			#print(args["plane_id"])
			print(str(state))
...
#missiles
	for name in missiles_g[:-4]:
		...
		if flag_print_log:
			...
			print(str(state))
		all_state.append(state)
	socket_lib.send_message(str.encode(json.dumps(all_state)))
