...
class TargettingDevice(MachineDevice):
    ...
    def update_target_flag(self):
        if len(self.targets) != 0:
            self.targets[self.target_id - 1].targeted += 1

    def at_advantage_angle(self) -> bool:
        return True
...
class AircraftAutopilotControlDevice(ControlDevice):
    ...
    def get_autopilot_heading(self) -> float:
        return self.autopilot_heading

    def set_autopilot_heading(self, value):
        self.autopilot_heading = max(min(360, value), 0)

    def get_autopilot_altitude(self) -> float:
        return self.autopilot_altitude
...
class AircraftIAControlDevice(ControlDevice):
    ...
    def __init__(self, name, machine, inputs_mapping_file, control_mode=ControlDevice.CM_KEYBOARD, start_state=False):
        ...
        self.FSM_machine = fm.Entity(self)
    ...
    def get_target_distance(self) -> float:
        ...
        if target is None:
            return -1.0
        this_pos = self.machine.parent_node.GetTransform().GetPos()
        target_pos = target.parent_node.GetTransform().GetPos()
        return hg.Len(this_pos - target_pos)

    def get_target(self):
        """获取目标"""
        td = self.machine.get_device("TargettingDevice")
        if td is not None and td.target_id != 0:
            return td.targets[td.target_id - 1]
        else:
            return None
    ...
    def fire_missile(self):
        ...
        """等待目标被锁定"""
        if td.target_locked and md is not None:
            md.fire_missile()
            self.machine.action_map["fire_missile"] = 0

    def update_IA_fight_fsm(self, aircraft, dts):
        ...
        if autopilot is not None:
            ...
            aircraft.set_brake_level(0)
            if speed < aircraft.minimum_flight_speed:  # 不能低于最低速度
                ...
                if td.target_id > 0:
                    if self.IA_flag_position_correction:  # 飞离战场纠正
                        ...
                        if target_distance < self.IA_target_distance_fight:
                            self.IA_flag_go_to_target = False
                            if self.FSM_machine.state.missile_freeze_timer <= 0:
                                self.fire_missile()
                                self.FSM_machine.state.missile_freeze_timer = self.FSM_machine.state.missile_freeze
                        ...
                        if aircraft.playfield_distance > aircraft.playfield_safe_distance:
                            ...
                            self.IA_position_correction_heading = aircraft.calculate_heading(
                                hg.Normalize(v * hg.Vec3(1, 0, 1)))
    ...
    def update_IA_escape_fsm(self, aircraft, dts):
        autopilot = aircraft.devices["AutopilotControlDevice"]
        if autopilot is not None:
            """调整速度"""
            autopilot.set_autopilot_speed(-1)
            ...
            if speed < aircraft.minimum_flight_speed:  # 不能低于最低速度
                ...
                if td.target_id > 0:
                    if self.IA_flag_position_correction:  # 飞离战场纠正
                        ...
                        """远离目标"""
                        target_distance = self.get_target_distance()
                        autopilot.set_autopilot_heading(td.target_heading + 180)
                        """检测是否飞离战场"""
                        if aircraft.playfield_distance > aircraft.playfield_safe_distance:
                            v = aircraft.parent_node.GetTransform().GetPos() * -1
                            self.IA_position_correction_heading = aircraft.calculate_heading(
    ...
    def update_IA_maneuver_fsm(self, aircraft, dts):
        if not self.IA_flag_maneuver_prepared:
            ...
            aircraft.activate_post_combustion()
            ready = True
            if not -1 < aircraft.pitch_attitude < 1:
                sign = 1 if aircraft.pitch_attitude > 0 else -1
                aircraft.set_pitch_level(0.1 * sign)
                ready = False
            if not -1 < aircraft.roll_attitude < 1:
                sign = 1 if aircraft.roll_attitude > 0 else -1
                aircraft.set_roll_level(-0.1 * sign)
                ready = False
            self.IA_flag_maneuver_prepared = ready
            if ready:
                self.IA_flag_maneuver_progress = 0
            return
        ...
        if self.IA_flag_maneuver_method == 0:
        ...
        elif self.IA_flag_maneuver_method == 1:
            aircraft.set_roll_level(-1)
            if self.IA_flag_maneuver_progress == 0:
                print("横滚机动")
                self.IA_flag_maneuver_progress = 1
            elif self.IA_flag_maneuver_progress == 1 and aircraft.roll_attitude < 0:
                self.IA_flag_maneuver_progress = 2
            ...
            if self.IA_flag_maneuver_progress == 0:
                print("眼镜蛇机动")
                self.IA_flag_maneuver_progress = 1
            elif self.IA_flag_maneuver_progress == 1 and aircraft.pitch_attitude > 80:
                self.IA_flag_maneuver_progress = 2
            elif self.IA_flag_maneuver_progress == 2 and aircraft.pitch_attitude < 65:
                self.IA_flag_maneuver_progress = 3
            elif self.IA_flag_maneuver_progress == 3 and aircraft.pitch_attitude < 0:
                aircraft.set_pitch_level(0)
                self.IA_flag_maneuver_progress = 0
    ...
    def update_IA_landing_fsm(self, aircraft, dts):
                            ...
                            if not self.IA_flag_reached_landing_point:
                                ...
                                if aircraft.ground_node_collision is not None:
                                    ...
                                    if hs < 1 and abs(vs) < 1:
                                        ...
                                        self.FSM_machine.trans_state(fs.FSMStateEnum.liftoff)
    ...
    def update_controlled_device(self, dts):
        aircraft = self.machine
        if not aircraft.wreck:
            self.FSM_machine._update(dts)
            self.machine.action_map = self.machine.action_map_default
        ...
        #    if self.IA_command == AircraftIAControlDevice.IA_COM_MANEUVER:
        #        self.update_IA_maneuver(aircraft, dts)

    def update_IA_liftoff_fsm(self, aircraft, dts):
        self.IA_flag_landing_target_found = False
        aircraft.set_flaps_level(1)
        if self.flag_IA_start_liftoff:  # 延迟执行
            self.IA_liftoff_delay -= dts
        if self.IA_liftoff_delay <= 0:
            if aircraft.thrust_level < 1 or not aircraft.post_combustion:  # 调整状态
                aircraft.set_brake_level(0)
                aircraft.set_flaps_level(1)
                aircraft.set_thrust_level(1)
                aircraft.activate_post_combustion()
            ...
            if aircraft.ground_node_collision is None:
                ...
                gear = aircraft.get_device("Gear")  # 收起落架
                if gear is not None:
                    if gear.activated:
                        gear.deactivate()
